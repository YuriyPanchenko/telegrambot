...
bot = telebot.TeleBot('5477453145:AAFYOnELYtsaIdwvs8xQuzANpQaW8HITZFw');

class DataBase:
	def __init__(self):
		cluster = MongoClient("mongodb+srv://Yuriy:Yuriy123!!!@cluster0.w4uk7.mongodb.net/?retryWrites=true&w=majority")
		...
		print("------------------------")
		print(len(list(self.questions.find({}))));
		print("------------------------")
...
def start(message):
	...
	if user["is_passing"]:
		bot.send_message(message.from_user.id, 'Тест ще триває')
		return
...
def get_question_message(user):
	if user["question_index"] == db.questions_count:
		...
		db.set_user(user["chat_id"], {"is_passed": False, "is_passing": False, "answers": []})
